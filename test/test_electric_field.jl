...
@testset "Cylindrical E field on a full-circle φ interval not starting at 0" begin
    T = Float64
    nr, nφ, nz = 6, 36, 5
    rmax, zmax = 0.05, 0.04
    A = 100.0  # V/m
    φ0 = π / 6
    axr = DiscreteAxis(0.0, rmax, :r0, :infinite, :closed, :closed, collect(range(0.0, rmax, length = nr)))
    axφ = DiscreteAxis(φ0, φ0 + 2π, :periodic, :periodic, :closed, :open, collect(range(φ0, φ0 + 2π, length = nφ + 1))[1:end-1])
    axz = DiscreteAxis(0.0, zmax, :infinite, :infinite, :closed, :closed, collect(range(0.0, zmax, length = nz)))
    grid = CylindricalGrid{T}((axr, axφ, axz))
    ...
    itp = SolidStateDetectors.interpolated_scalarfield(epot)
    for φ in (0.0, φ0, 2π - 0.01, φ0 + 2π + 0.01)  # incl. evaluation through the wrap
        @test itp(0.8 * rmax, φ, zmax / 2) ≈ A * 0.8 * rmax * cos(φ) rtol = 0.02
    end
end
