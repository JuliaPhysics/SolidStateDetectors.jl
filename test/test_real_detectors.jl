...
@timed_testset "Inverted Coax" begin
    ...
    @test isapprox(deplV, 2030*u"V", atol = 10.0*u"V") 
    ...
    sim.detector = SolidStateDetector(sim.detector, contact_id = id, contact_potential = deplV - deplV*0.005)
...
@timed_testset "Coax" begin
    ...
    timed_calculate_electric_potential!(sim, convergence_limit = 1e-5, device_array_type = device_array_type, refinement_limits = [0.2, 0.1], verbose = false)
    timed_calculate_electric_field!(sim)
    timed_calculate_weighting_potential!(sim, 1, convergence_limit = 1e-5, refinement_limits = [0.2, 0.1], verbose = false)
    timed_calculate_weighting_potential!(sim, 2, convergence_limit = 1e-5, refinement_limits = [0.2, 0.1], verbose = false)
    for i in 3:19
        timed_calculate_weighting_potential!(sim, i, convergence_limit = 1e-5, refinement_limits = [0.2], verbose = false)
    end
