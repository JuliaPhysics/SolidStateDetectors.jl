...
@testset "Test real detectors" begin
    @testset "Simulate example detector: Pseudo Geometry" begin
        sim = Simulation{T}(SSD_examples[:PseudoGeometry])
        simulate!(sim, convergence_limit = 1e-6, refinement_limits = [0.2, 0.1], verbose = false)
        evt = Event(CartesianPoint.([CartesianPoint{T}(0.0025,0.0025,0.004)]))
