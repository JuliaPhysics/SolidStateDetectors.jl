...
@testset "Run Geant4 simulations" begin
    ...
    @test_throws Exception SolidStateDetectors.cluster_detector_hits(evts_static, 2u"kg", 1u"ns")

    # Check clustering with cluster time being unitless or with the wrong unit
    clustered_evts_static_unitless = SolidStateDetectors.cluster_detector_hits(evts_static, 10u"µm", 1e-5)
    @test clustered_evts_static == clustered_evts_static_unitless
    @test_throws Exception SolidStateDetectors.cluster_detector_hits(evts_static, 10u"µm", 2u"kg")
