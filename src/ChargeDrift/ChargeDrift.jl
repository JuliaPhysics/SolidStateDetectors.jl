...
function _drift_charges( detector::SolidStateDetector{T}, grid::Grid{T, 3},
    ...
    for i in eachindex(starting_points)
        ...
        drift_charge!(drift_path_e, detector, grid, starting_points[i], Δt, velocity_field_e, verbose = verbose)
        drift_charge!(drift_path_h, detector, grid, starting_points[i], Δt, velocity_field_h, verbose = verbose)
...
function _drift_charge( detector::SolidStateDetector{T}, grid::Grid{T, 3},
                       starting_point::CartesianPoint{T}, 
                       velocity_field_e::Interpolations.Extrapolation{SVector{3, Float64}, 3},
                       velocity_field_h::Interpolations.Extrapolation{SVector{3, Float64}, 3}; 
                       Δt::T = T(1f-9), n_steps::Int = 2000, verbose::Bool = true)::Vector{DriftPath{T}} where {T <: SSDFloat}
    return _drift_charges(detector, grid, [starting_point], velocity_field_e, velocity_field_h, Δt = Δt, n_steps = n_steps, verbose = verbose)
end
