...
mutable struct CGD{T<:AbstractFloat} <: SolidStateDetector{T}
    ...
    bulk_geometry::Vector{AbstractGeometry{T}}
...
function CGD{T}(config_file::Dict)::CGD where {T <: AbstractFloat}
    ...
    det.charge_carrier_density_top = geom_round(T(config_file["crystal"]["charge_carrier_density"]["top"]))
    det.charge_carrier_density_bot = geom_round(T(config_file["crystal"]["charge_carrier_density"]["bot"]))
    
    n_contacts = Contact{T, :N}[ Contact{T, :N}( contact_dict, det.geometry_unit ) for contact_dict in config_file["contacts"]["n_contacts"] ]
    p_contacts = Contact{T, :P}[ Contact{T, :P}( contact_dict, det.geometry_unit ) for contact_dict in config_file["contacts"]["p_contacts"] ]
    det.contacts = vcat(n_contacts, p_contacts)
    return det
end
...
function Grid(  det::CGD{T}; 
    ...
    int_x::Interval{:closed, :closed, T} = Interval{:closed, :closed, T}(det.bulk_geometry[1].x[1] - 0.005, det.bulk_geometry[1].x[2] + 0.005)
    int_y::Interval{:closed, :closed, T} = Interval{:closed, :closed, T}(det.bulk_geometry[1].y[1] - 0.005, det.bulk_geometry[1].y[2] + 0.005)
    int_z::Interval{:closed, :closed, T} = Interval{:closed, :closed, T}(det.bulk_geometry[1].z[1] - 0.005, det.bulk_geometry[1].z[2] + 0.005)
...
function in(pt::AnyPoint{T}, det::CGD{T})::Bool where {T}
    return in(pt, det.bulk_geometry)
end

function get_charge_density(detector::SolidStateDetector{T}, pt::CartesianPoint{T})::T where {T}
    ...
    crystal_length_x::T = detector.bulk_geometry[1].x[2] - detector.bulk_geometry[1].x[1]
