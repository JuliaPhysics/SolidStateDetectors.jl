...
function get_precision_type(d::SolidStateDetector)::Type
        return typeof(d).parameters[1]
end
...
function get_important_φ_points(d::SolidStateDetector)
    T = get_precision_type(d)
...
# false -> outside
function contains(c::Coax, r::Real, ϕ::Real, z::Real)::Bool
    rv::Bool = true
    if !check_outer_limits(c, r, ϕ, z) rv = false end
    if !check_borehole(c, r, ϕ, z) rv = false end
    if !check_tapers(c, r, ϕ, z) rv = false end
    return rv
end
...
function contains(b::BEGe, r::Real, ϕ::Real, z::Real)::Bool
    rv::Bool = true
    check_outer_limits(b,r,ϕ,z) ? nothing : rv = false
    check_tapers(b,r,ϕ,z) ? nothing : rv = false
    check_grooves(b,r,ϕ,z) ? nothing : rv = false
    rv
end
...
function contains(ivc::InvertedCoax,r::Real, ϕ::Real, z::Real)::Bool
    rv::Bool = true
    check_outer_limits(ivc,r,ϕ,z) ? nothing : rv = false
    check_tapers(ivc,r,ϕ,z) ? nothing : rv = false
    if ivc.borehole_modulation == true
        check_borehole(ivc,r,ϕ,z,ivc.borehole_ModulationFunction) ? nothing : rv = false
    else
        check_borehole(ivc,r,ϕ,z) ? nothing : rv = false
    end
    check_grooves(ivc,r,ϕ,z) ? nothing : rv = false
    rv
end
...
function check_outer_limits(d::SolidStateDetector, r::Real, ϕ::Real, z::Real)::Bool
    rv::Bool = true
...
function check_borehole(c::Coax, r::Real, ϕ::Real, z::Real)::Bool
    rv = true
...
function check_borehole(ivc::InvertedCoax, r::T, φ::T, z::T,modulation_function::Function)::Bool where T<:Real#returns true if point is not inside borehole
    ...
    if r < round(ivc.borehole_radius+modulation_function(φ),digits=6) && z >round(ivc.crystal_length-ivc.borehole_length,digits=6)
        rv = false
    end
    rv
end
...
function check_tapers(ivc::InvertedCoax, r::Real, φ::Real, z::Real)::Bool
    rv::Bool = true
...
function check_tapers(c::Coax, r::Real, ϕ::Real, z::Real)::Bool
...
function check_grooves(b::BEGe,r::Real,ϕ::Real,z::Real)::Bool
    # T = get_precision_type(b)
    rv::Bool=true
...
function check_grooves(ivc::InvertedCoax,r::Real,ϕ::Real,z::Real)::Bool
    T=typeof(z)
    rv::Bool=true
...
# is_boundary
# function is_boundary_point(d::SolidStateDetector, r::Real, ϕ::Real, z::Real)::Bool
#     rv::Bool = false
#     for iseg in 1:size(d.segment_bias_voltages,1)
#         if (d.segmentation_r_ranges[iseg][1] <= r <= d.segmentation_r_ranges[iseg][2])
#             if (d.segmentation_phi_ranges[iseg][1] <= ϕ <= d.segmentation_phi_ranges[iseg][2])
...
#     rv
# end
# function is_boundary_point(d::Coax, r::Real, ϕ::Real, z::Real, rs, φs, zs)
#     is_boundary_point(d,r,ϕ,z)
# end
...
function point_type(d::SolidStateDetector, r::T, φ::T, z::T) where {T<:AbstractFloat}
    T==Float32 ? atol = 0.000001 : atol = 0.000000000000001
    rv::Symbol = :bulk
    !contains(d, Cylindrical{T}(r,φ,z)) ? rv = :outside : nothing
    i=0
    digits::Int=6
    ############################# Electrode Definitions
    for iseg in 1:size(d.segment_bias_voltages,1)
        if d.borehole_modulation == true && iseg == d.borehole_segment_idx
             x = (round(d.segmentation_r_ranges[iseg][1]+d.borehole_ModulationFunction(φ),digits=digits) <= r <= round(d.segmentation_r_ranges[iseg][2]+d.borehole_ModulationFunction(φ),digits=digits))
        elseif d.borehole_modulation == true && iseg == d.borehole_bot_segment_idx
             x = (d.segmentation_r_ranges[iseg][1] <= r <= round(d.segmentation_r_ranges[iseg][2]+d.borehole_ModulationFunction(φ),digits=digits))
        elseif d.borehole_modulation == true && iseg == d.borehole_top_segment_idx
            x = (round(d.segmentation_r_ranges[iseg][1]+d.borehole_ModulationFunction(φ),digits=digits) <= r <= d.segmentation_r_ranges[iseg][2])
        else
            x = (d.segmentation_r_ranges[iseg][1] <= r <= d.segmentation_r_ranges[iseg][2])
        end
        if x
            if (d.segmentation_phi_ranges[iseg][1] <= φ <= d.segmentation_phi_ranges[iseg][2])
                if (d.segmentation_z_ranges[iseg][1] <= z <= d.segmentation_z_ranges[iseg][2])
                    if d.segmentation_types[iseg]=="Tubs"
                        ...
                        if isapprox(analytical_taper_r_from_φz(φ,z,
                            d.segmentation_types[iseg],
                            d.segmentation_r_ranges[iseg],
                            d.segmentation_phi_ranges[iseg],
                            d.segmentation_z_ranges[iseg]
...
# get_potential
function get_boundary_value(d::SolidStateDetector, r::Real, ϕ::Real, z::Real, rs::Vector{<:Real})::get_precision_type(d)
    if d.borehole_modulation==true
        res::get_precision_type(d)=0.0
        try
            res = d.segment_bias_voltages[ get_segment_idx(d, r, ϕ, z, rs) ]
        catch
            println("kek")
            @show r, ϕ, z
        ...
        return d.segment_bias_voltages[ get_segment_idx(d, r, ϕ, z) ]
    end
end



function get_charge_density(detector::SolidStateDetector, r::Real, ϕ::Real, z::Real)::get_precision_type(detector)
