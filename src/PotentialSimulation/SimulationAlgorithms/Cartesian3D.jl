...
#     nothing
# end
# function apply_boundary_conditions_on_φ_axis!(  rbpot::Array{T, 4}, iz::Int, ir::Int, rbi::Int, ax::DiscreteAxis{T, :periodic, :periodic}, int::Interval{:closed, :closed, T},
#                                                 grid_boundary_factors::NTuple{2, T})::Nothing where {T}
#     rbpot[iz,   1, ir, rbi] = rbpot[ iz, end - 1, ir, rbi] # cycling boundary
#     rbpot[iz, end, ir, rbi] = rbpot[ iz,       2, ir, rbi] # cycling boundary
#     nothing
...
function apply_boundary_conditions!(fssrb::PotentialSimulationSetupRB{T, N1, N2, :Cartesian}, update_even_points::Val{even_points}, only2d::Val{only_2d}) where {T, N1, N2, even_points, only_2d}
    ...
    #         apply_boundary_conditions_on_φ_axis!( fssrb.potential, iz, ir, rbi, fssrb.grid.axes[2], fssrb.grid.axes[2].interval, fssrb.grid_boundary_factors[2])
    #     end
    # end
