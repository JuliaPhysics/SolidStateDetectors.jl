@fastmath function middleloop!( ir::Int, rb_tar_idx::Int, rb_src_idx::Int, 
                                pssrb::PotentialSimulationSetupRB{T, 3, 4, Cylindrical},
    ...
    @inbounds begin 
        ...
        pwwrr::T               = pssrb.geom_weights[1].weights[1, inr]
        pwwrl::T               = pssrb.geom_weights[1].weights[2, inr]
        r_inv_pwΔmpr::T        = pssrb.geom_weights[1].weights[3, inr]
        Δr_ext_inv_r_pwmprr::T = pssrb.geom_weights[1].weights[4, inr] 
        Δr_ext_inv_l_pwmprl::T = pssrb.geom_weights[1].weights[5, inr] 
        Δmpr_squared::T        = pssrb.geom_weights[1].weights[6, inr]  
        ...
        for iφ in 2:2:(size(pssrb.potential, 2) - 1)
            ...
            pwwφr::T        = pssrb.geom_weights[2].weights[1, inφ]
            pwwφl::T        = pssrb.geom_weights[2].weights[2, inφ]
            pwΔmpφ::T       = pssrb.geom_weights[2].weights[3, inφ]
            Δφ_ext_inv_r::T = pssrb.geom_weights[2].weights[4,  iφ]
            Δφ_ext_inv_l::T = pssrb.geom_weights[2].weights[4, inφ]
...
function load_weights_for_innerloop!(line_weights, pssrb::PotentialSimulationSetupRB{T, 3, 4, Cylindrical},
                    iφ, inφ, ir, inr,
...
function innerloop!(line_weights, pssrb::PotentialSimulationSetupRB{T, 3, 4, Cylindrical}, 
