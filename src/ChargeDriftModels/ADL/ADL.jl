...
@fastmath function setup_gamma_matrices(phi110::T)::SVector{4, SMatrix{3,3,T}} where {T <: SSDFloat}
    ml::T = 1.64
    mt::T = 0.0819
...
function get_electron_drift_field(ef::Array{SVector{3, T},3}, chargedriftmodel::ADLChargeDriftModel)::Array{SVector{3,T},3} where {T <: SSDFloat}
    ...
    @fastmath function getVe(fv::SVector{3, T}, gammas::SVector{4, SMatrix{3,3,T}})::SVector{3, T} where {T <: SSDFloat}
        @inbounds begin
            Emag::T = norm(fv)
            Emag_inv::T = inv(Emag)
...
function get_hole_drift_field(ef::Array{SVector{3,T},3}, chargedriftmodel::ADLChargeDriftModel)::Array{SVector{3,T},3} where {T <: SSDFloat}
    ...
    @fastmath function getVh(fv::SVector{3,T}, Emag_threshold::T)::SVector{3,T} where {T <: SSDFloat}
        @inbounds begin
            Emag::T = norm(fv)
            Emag_inv::T = inv(Emag)
