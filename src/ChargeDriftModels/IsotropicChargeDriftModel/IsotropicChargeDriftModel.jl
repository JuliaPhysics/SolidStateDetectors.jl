...
@fastmath function getVe(fv::SVector{3, T}, cdm::IsotropicChargeDriftModel{T}, current_pos::CartesianPoint{T}) where {T <: SSDFloat}
    @inbounds begin 
        -cdm.μ_e*fv
    end
end

@fastmath function getVh(fv::SVector{3, T}, cdm::IsotropicChargeDriftModel{T}, current_pos::CartesianPoint{T}) where {T <: SSDFloat}
    @inbounds begin 
        cdm.μ_h*fv
    end
end