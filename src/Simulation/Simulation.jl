...
function apply_initial_state!(sim::Simulation{T}, ::Type{ElectricPotential}, grid::Grid{T} = Grid(sim);
    ...
    sim.ϵ_r = DielectricDistribution(DielectricDistributionArray(pssrb), grid)
...
function refine!(sim::Simulation{T}, ::Type{ElectricPotential},
    ...
    if update_other_fields
        ...
        sim.ϵ_r = DielectricDistribution(DielectricDistributionArray(pssrb), sim.electric_potential.grid)
...
function drift_charges( sim::Simulation{T}, starting_positions::Vector{CartesianPoint{T}}, energies::Vector{T};
                        Δt::RealQuantity = 5u"ns", max_nsteps::Int = 1000, self_repulsion::Bool = false, verbose::Bool = true )::Vector{EHDriftPath{T}} where {T <: SSDFloat}
    return _drift_charges(   sim.detector, sim.point_types.grid, sim.point_types, starting_positions, energies, 
                             ...
                             max_n_steps = max_n_steps, self_repulsion = self_repulsion, verbose = verbose)
end
