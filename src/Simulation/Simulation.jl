...
# * the undepleted bits (via `mark_undep_bits!`, from `sim.imp_scale`).
#
# ## Arguments
    ...
    if refine
        for iref in 1:n_refinement_steps
            ...
            if isEP
                ...
                refine!(sim, ElectricPotential, max_diffs, min_tick_distance, depletion_handling = depletion_handling)
