            ...
            for ir in 1:size(ef, 1)
                ...
                if pointtypes[ir, iθ, iz] & update_bit == 0 # boundary points 
                    if (1 < ir < size(pointtypes, 1)) 
                        if (pointtypes[ir - 1, iθ, iz] & update_bit > 0) && (pointtypes[ir + 1, iθ, iz] & update_bit > 0) 
                            er = 0 
                        elseif (pointtypes[ir - 1, iθ, iz] & update_bit > 0) || (pointtypes[ir + 1, iθ, iz] & update_bit > 0) 
                            er *= 2
                        end 
                    end
                    if (1 < iθ < size(pointtypes, 2)) 
                        if (pointtypes[ir, iθ - 1, iz] & update_bit > 0) && (pointtypes[ir, iθ + 1, iz] & update_bit > 0) 
                            eθ = 0 
                        elseif (pointtypes[ir, iθ - 1, iz] & update_bit > 0) || (pointtypes[ir, iθ + 1, iz] & update_bit > 0) 
                            eθ *= 2
                        end 
                    end
                    if (1 < iz < size(pointtypes, 3)) 
                        if (pointtypes[ir, iθ, iz - 1] & update_bit > 0) && (pointtypes[ir, iθ, iz + 1] & update_bit > 0) 
                            ez = 0 
                        elseif (pointtypes[ir, iθ, iz - 1] & update_bit > 0) || (pointtypes[ir, iθ, iz + 1] & update_bit > 0) 
                            ez *= 2 
                        end 
                    end
...
function get_electric_field_from_potential(ep::ElectricPotential{T, 3, :Cartesian}, pointtypes::PointTypes{T})::Array{SArray{Tuple{3},T,1,3}, 3} where {T <: AbstractFloat} 
    ...
    axθ::Vector{T} = collect(ep.grid[:y])
    axz::Vector{T} = collect(ep.grid[:z])
    axr_ext::Vector{T} = get_extended_ticks(ep.grid[:x])
    axθ_ext::Vector{T} = get_extended_ticks(ep.grid[:y])
