...
@inline _parse_interval(::Type{T}, x::Union{Real, String}, unit::Unitful.Units) where {T} = _parse_value(T, x, unit)
function _parse_interval(::Type{T}, dict::Union{Dict{String,Any}, Dict{Any,Any}}, unit::Unitful.Units) where {T}
    @assert haskey(dict, "from") && haskey(dict, "to") "Please specify 'from' and 'to' in $(dict)"
    _parse_interval_from_to(T, dict, unit)
end

#parses dictionary entry for heights to respective AbstractFloat/Interval (divides by 2 if it is a single value)
@inline _parse_interval_height(::Type{T}, x::Union{Real, String}, unit::Unitful.Units) where {T} = _parse_value(T, x, unit)/2
@inline _parse_interval_height(::Type{T}, dict::Union{Dict{String,Any}, Dict{Any,Any}}, unit::Unitful.Units) where {T} = _parse_interval(T, dict, unit)


# parses dictionary entry for φ-interval that has {"from" ..., "to": ...} to the respective Nothing/Interval
function _parse_interval_φ(::Type{T}, dict::Union{Dict{String,Any}, Dict{Any,Any}}, unit::Unitful.Units) where {T}
...
function parse_r_of_primitive(::Type{T}, dict::Union{Dict{String,Any}, Dict{Any,Any}}, unit::Unitful.Units) where {T}
    ...
    if haskey(dictr, "bottom") && haskey(dictr, "top")
        bottom = _parse_interval(T, dictr["bottom"], unit)
        top = _parse_interval(T, dictr["top"], unit)
...
function parse_φ_of_primitive(::Type{T}, dict::Union{Dict{String,Any}, Dict{Any,Any}}, unit::Unitful.Units) where {T}
    φ = if !haskey(dict,"phi")
        ...
        _parse_interval_φ(T, dict["phi"], unit)
    end
end
...
function parse_height_of_primitive(::Type{T}, dict::Union{Dict{String,Any}, Dict{Any,Any}}, unit::Unitful.Units) where {T}
    ...
    haskey(dict,"h") ? _parse_interval_height(T, dict["h"], unit) : _parse_interval_height(T, dict["z"], unit)
end

# converts the content of the dictionary to respective AbstractFloat/Interval. Numbers will not be divided by 2!
