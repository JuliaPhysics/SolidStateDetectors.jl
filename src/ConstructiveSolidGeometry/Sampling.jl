...
sample(tg::TranslatedGeometry{T}, g::CartesianTicksTuple{T}) where {T} = translate!(sample(tg.p, (x = g.x .- tg.t[1], y = g.y .- tg.t[2], z = g.z .- tg.t[3])), tg.t)
sample(tg::TranslatedGeometry{T}, g::CylindricalTicksTuple{T}) where {T} = sample(tg.p, (r = g.r, φ = g.φ, z = g.z .- tg.t[3])) .+ (CartesianVector{T}(0,0,tg.t[3]),)
...
sample_surface(tg::TranslatedGeometry{T}, g::CartesianTicksTuple{T}) where {T} = translate!(sample_surface(tg.p, (g[1] .- tg.t[1], g[2] .- tg.t[2], g[3] .- tg.t[3])), tg.t)
sample_surface(tg::TranslatedGeometry{T}, g::CylindricalTicksTuple{T}) where {T} = sample_surface(tg.p, (g[1], g[2], g[3] .- tg.t[3])) .+ (CartesianVector{T}(0,0,tg.t[3]),)
