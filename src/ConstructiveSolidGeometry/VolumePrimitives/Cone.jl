...
function Cone(rtop::R1, rbot::R2, height::H) where {R1<:Real, R2<:Real, H<:Real}
...
@inline get_r_at_z(rtop::TR, rbot::TR, cz::TZ, z::Real) where {TR, TZ} = (rtop - rbot) * (z - _left_linear_interval(cz)) / _width_linear_interval(cz) + rbot
