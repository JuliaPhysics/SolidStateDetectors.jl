function distance_to_surface(point::AbstractCoordinatePoint{T}, c::Union{<:AbstractConstructiveGeometry{T}, <:AbstractVolumePrimitive{T}}) where {T}
    ...
    @assert isfinite(min_distance) "Couldn't determine the distance to surface from this point. Something unexpected issue happens with the geometry or this point."
    return min_distance
end
