...
function vertices(cm::ConeMantle{T}, spacing::T)::Vector{CartesianPoint{T}} where {T}
    ...
    z = range(-cm.hZ, cm.hZ, length = max(2,Int(ceil(l/spacing))))
    [_transform_into_global_coordinate_system(CartesianPoint{T}((radius_at_z(cm,z) .* reverse(sincos(φ)))..., z), cm) for z in z for r in [radius_at_z(cm,z)] for φ in (r == 0 ? [φMin] : range(φMin, φMax - full2π*spacing/radius_at_z(cm,z), length = max(2,Int(ceil(Δφ*radius_at_z(cm,z)/spacing)))))]
end
