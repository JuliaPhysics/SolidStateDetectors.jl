...
function vertices(es::EllipticalSurface{T}, spacing::T)::Vector{CartesianPoint{T}} where {T}
    ...
    [_transform_into_global_coordinate_system(CartesianPoint{T}(r*cos(φ), r*sin(φ), 0), es) for r in range(rMin, rMax, length = max(2,Int(ceil((rMax-rMin)/spacing)))) for φ in (r == 0 ? [φMin] : range(φMin, φMax - full2π*spacing/r, length = max(2,Int(ceil(Δφ*r/spacing)))))]
end
