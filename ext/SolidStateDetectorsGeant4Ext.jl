...
function SolidStateDetectors.run_geant4_simulation(app::G4JLApplication, number_of_events::Int; threshold = eps(Float64)*u"keV")
    evts = DetectorHit[]
    ...
    while evtno <= number_of_events
        out = missing
	while ismissing(out) || sum(out.edep) < threshold      
